...
#define DRIVER_NAME "BRAM_KERNEL_DRIVER"
#define DEVICE_NAME "BRAM_KERNEL"
...
static struct platform_driver BRAM_KERNEL_driver = {
...
ssize_t BRAM_read (struct file *pfile, char __user *buf, size_t length, loff_t *offset){
	...
	for(i=0; i<9; i++)
	{
		value  = ioread32(bp->base_addr + i*4);
...
static int __init BRAM_init(void)
{
	...
	my_device = device_create(my_class, NULL, my_dev_id, NULL, "bram_kernel");

	if(my_device == NULL){
		printk(KERN_ERR "Failde to create device BRAM_kernel\n");
		goto fail_1;
	}
	printk(KERN_INFO "created BRAM: bram_kernel\n");
	...
	printk(KERN_INFO "Hello from BRAM_KERNEL_DRIVER\n");

	return platform_driver_register(&BRAM_KERNEL_DRIVER);
...
static void __exit BRAM_exit(void)
{
	...
	platform_driver_unregister(&BRAM_KERNEL_DRIVER);
