...
#define DRIVER_NAME "BRAM_AFTER_CONV_DRIVER"
#define DEVICE_NAME "BRAM_AFTER_CONV"
...
static struct platform_driver BRAM_AFTER_CONV_driver = {
...
ssize_t BRAM_write (struct file *pfile, const char __user *buf, size_t length, loff_t *offset){
	...
	printk(KERN_INFO "Cannot write in this file\n");
	return 0;
}
...
static int __init BRAM_init(void)
{
	...
	my_device = device_create(my_class, NULL, my_dev_id, NULL, "bram_after_conv");

	if(my_device == NULL){
		printk(KERN_ERR "Failde to create device BRAM_after_conv\n");
		goto fail_1;
	}
	printk(KERN_INFO "created BRAM: BRAM_after_conv\n");
