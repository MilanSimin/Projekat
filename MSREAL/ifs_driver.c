...
static int ifs_probe (struct platform_device *pdev){
	...
	if(!r_mem ){
		printk(KERN_ALERT "IFS_probe: Failed to get resource\n");
		return -ENODEV;
	}
	// Get memory for structure ifs_info
	ip= (struct ifs_info *) kmalloc (sizeof(struct ifs_info),GFP_KERNEL);
	if (!ip) {
		printk(KERN_ALERT "could not allocate ifs device\n");
		return -ENOMEM;
	}
	// Put phisical adresses in ifs_info structure
	...
	printk(KERN_NOTICE "IFS platform driver registered \n");
	return 0;
