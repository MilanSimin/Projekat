...
void sendKernel(int sockfd)
{
	
	read(sockfd, &kernel, sizeof(int)*9);
...
void ReadLinesColumns(int sockfd)
{
	read(sockfd, &lines, sizeof(int));
...
void SendFileToClient(int sockfd)
{
	...
	write(sockfd,pom2,sizeof(int)*num);
				...
				string command;
					...
					do {
						...
						if(command == "y\n"){
							...
							sendKernel(newsockfd);
							ReadImage(newsockfd);
							ReadLinesColumns(newsockfd);
							ReadAfterConv();
